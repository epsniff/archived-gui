...
import (
	...
	"github.com/epsniff/gui/src/server/scheduler/actor/placement/hash"
...
func TestPeerOptimisticallyLive(t *testing.T) {
	...
	tr := New()
	tr.CreateActorPool("pool1", &hash.HashPlacement{})
	tr.CreateActorPool("pool2", &hash.HashPlacement{})
	for p := range peers {
		tr.OptimisticallyLive(p)
	}

	if err := tr.Register("pool1", true, grid.NewActorStart("worker1"), "peer1"); err != nil {
		t.Fatalf("err:%v", err)
	}
